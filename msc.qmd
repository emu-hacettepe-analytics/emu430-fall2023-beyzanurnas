...
\begin{split} 
\begin{align}
&\text{minimize} \sum_{j \epsilon N} y_{j} \\
&\text{st} \\ 
&\sum_{j \epsilon N} x_{ij} = 1 \qquad \qquad \qquad\ \ \forall  i \epsilon N \\
&y_{j} \ge x_{ij} \qquad \qquad \qquad \qquad \forall  i \epsilon N, j \epsilon N \\
&\sum_{j \epsilon N} x_{ij}d_{ij} \le Dmax \qquad \quad\ \forall  i \epsilon N\\
& x_{ij}, y_{j} \; \epsilon \; \{0,1\} \qquad \qquad \qquad \forall i \epsilon N, j \epsilon N \\
\end{align}
\end{split}
$$
...
for i in range(N):
    plt.scatter(nLocs[i][0], nLocs[i][1], color = 'black', s = 2)
```
